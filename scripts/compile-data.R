...
bili_rep_scores <- read_delim('pipeline_scores/p_logtbilw0_replication_scores.txt', delim = '\t') %>% 
  mutate(dat = 'Original data', type = 'Baseline bilirubin levels') %>% 
  select(-Replicate)
bili_per_scores <- read_delim('pipeline_scores/p_logtbilw0_permutation_scores.txt', delim = '\t') %>% 
  mutate(dat = 'Permuted data', type = 'Baseline bilirubin levels') %>% 
  select(-Permutation)
...
bilirubin_feats <- readxl::read_xlsx('supplementary_data/bilirubin_feature_importances.xlsx') %>% 
  mutate(type = 'Baseline bilirubin levels')
...
ttest_label <- tribble(
  ...
  test_r2(bili_rep_scores, bili_per_scores), test_mse(bili_rep_scores, bili_per_scores), 'Baseline bilirubin levels', -0.0425,
