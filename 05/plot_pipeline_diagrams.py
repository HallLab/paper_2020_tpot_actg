...
def plot_pipeline_diagrams(phenotype):
    ...
    # Read data
    n = 10
    replication_results = pd.read_table(f'../04/{phenotype}_replication_scores.txt', sep="\t").head(n)
    ...
    graph.write_png(f"figs/{phenotype}_pipeline_diagram.png")
    graph.write_svg(f"figs/{phenotype}_pipeline_diagram.svg")

if __name__ == '__main__':
    plot_pipeline_diagrams("p_cd4difw48w4")
    plot_pipeline_diagrams("vllogdifw48w4")
